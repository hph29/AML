...
def intializeClusters(X, C, kmeans = False):
    N = X.shape[0]
    d = X.shape[1]
...
def gaussEM(X, C, niters = 100, kmeans = False):
    N = X.shape[0]
...
if __name__ == '__main__':
    ...
    for i in [10, 20, 50]:
        classes, result = gaussEM(X, i, niters = 50, kmeans = False)
